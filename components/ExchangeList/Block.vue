<template>
  <div :class="`exchange-block flex basis-1/2`">
    <div v-if="thisCourse() !== '0'" :class="`flex flex-col size-full gap-2.5  ${reverse ? 'self-end' : 'self-start'}`">
      <span v-if="type === 'Primary'" :class="`text-white text-opacity-50 ${reverse ? 'mr-auto' : 'ml-auto'}`">{{ title
        }}</span>
      <div
        :class="`exchange-currency-block gap-5 items-center ${reverse ? 'flex-row-reverse self-start' : 'self-end'}`">
        <span v-if="type === 'Primary'" class="text-base font-semibold text-white text-opacity-50 text-nowrap">{{
          breakText(selectedItem, 11) }}</span>
      ...
      <span :class="`title-2 text-nowrap ${reverse ? 'mr-auto' : 'ml-auto'}`"
