...
function filterCurrency() {
    return selectedType.value === curr.attributes.out_currency?.currency_type ? ''
        : selectedType.value === '_all' ? '' : 'hidden', store.getSelectedItem(isGetChanger).id === curr.id ? 'active' : ''
}
...
<template>
    <div class="wrapper h-full">
        <label for="exchange" class="exchange-body-input-wrapper" @click="focusElement($event, 'input_' + id)">
            <misc-b-input v-if="id === 2" input-type="primary" placeholder="Введите сумму"
                append-icon="Bitcoin_thumbnail" :prefix="store.selectedGiveItem.code" is-icon-grayscale="true"
                v-model:="store.calculatedGive.value" />
            <misc-b-input v-if="id === 3" v-model="store.exchanger.get" input-type="primary" placeholder="Введите сумму"
                append-icon="Bitcoin_thumbnail" :prefix="store.selectedGetItem.code" is-icon-grayscale="true" />
            <transition name="fade">
                <small v-if="id === 3" v-show="store.currentTab == 1">
                    {{ 'Курс: ' +
                        store.selectedGetItem.course.toFixed(store.selectedGetItem.decimals).toLocaleString("ru-RU")
                        + ' ' + store.selectedGetItem.code }}</small>
            </transition>
            <transition name="fade">
                <small v-if="id === 3" v-show="store.currentTab == 0">
                    {{ 'Резерв: ' + store.selectedGetItem.reserve.toLocaleString("ru-RU") + ' ' +
                        store.selectedGetItem.code
                        + '' ?? store.details.res.attributes.max_out }}</small>
            </transition>
            <transition name="fade">
                <small v-if="id === 2">
                    От <span class="link"
                        :class="store.exchanger.to < store.details.res?.attributes?.min_in ? 'danger' : ''">
                        {{ store.details.res?.attributes?.min_in + ' ' +
                            store.selectedGiveItem.code
                        }}</span>
                    до <span class="link"
                        :class="store.exchanger.to > store.details.res?.attributes?.max_in ? 'danger' : ''">
                        {{
        ...
        <div class="exchange-body-currency-wrapper" @click="focusElement($event, 'search_' + id)">
            <misc-b-input v-model="searchValue" input-type="search" prepend-icon="search" placeholder="Найти валюту" />
            <div class="exchange-body-currency-tabs">
                <b-button v-for="curr in CURRENCY" :key="curr.id" :text="curr.title" size="XS" type="Tab"
                    :class="selectedType === curr.id ? 'active' : ''" @click="filterByType(curr.id)" />
            </div>
            <!-- currency list GIVE -->
            <div v-if="id === 2" :id="'exchange-' + id">
                <misc-spinner v-if="!store.currency || !store.currencyOperations" />
                <div v-if="store.currency && store.currencyOperations" class="exchange-body-currency-items">
                    <exchange-list-body-item v-for="curr in store.currency.data" :key="curr.id" :content="curr" />
                </div>
            </div>
            <!-- currency list GET -->
            <div v-if="id === 3" :id="'exchange-' + id">
                <misc-spinner v-if="!store.currency || !store.currencyOperations" />
                <div v-if="store.cleanedCurrencyList" class="exchange-body-currency-items">
                    <div v-for="curr in store.cleanedCurrencyList" v-show="!!searchContent ? curr.attributes.out_currency.payment_name.toLowerCase().includes(searchContent)
                        || curr.attributes.out_currency.iso_code.toLowerCase().includes(searchContent) : true"
                        :id="curr.id" :key="curr.id" class="currency group" :class="[
                            filterCurrency,
